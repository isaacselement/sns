...
<title>${user.username} I微博  - 随时随地分享身边的新鲜事儿</title>
	...
	<table width="850px" height="1156" border="0" align="center"
		...
		<tr>
			<td width="74%" height="181" valign="top" nowrap="nowrap"
				...
				<div style="text-align: center">
					<s:set name="flag" value="flag"></s:set>
					<s:if test="#flag!=0">
						<input id="addfollowing<s:property value='user.id'/>"
							name="addfollowing<s:property value='#c.id'/>" type="button"
					...
					<s:elseif test="#flag==0">
						<input id="addfollowing<s:property value='user.id'/>"
							...
							onclick="addFollowing('following<s:property value="user.id"/>',<s:property value="user.id"/>)" />
					</s:elseif>
				</div>
			...
			<td width="26%" rowspan="2" valign="top"
				style="text-align: left"><h2>
					<span style=""><a
						href="<%=request.getContextPath()%>/following.action?followingid=<s:property value="user.id"/>""><span
							class="k"><s:if test="#sex==true">他的</s:if>
					<s:else>她的</s:else>关注</span>
					</a>：</span><span class="number">${followingsNum}</span>
				</h2>
				<h2>
					<a
						href="<%=request.getContextPath()%>/follow.action?followersid=<s:property value="user.id"/>"><span
						class="k"><s:if test="#sex==true">他的</s:if>
					<s:else>她的</s:else>粉丝</span>
					</a>：<span class="number">${followersNum}</span>
				</h2>
				<h2>
					<a href="<%=request.getContextPath()%>/profile.action"><span
						class="k"><s:if test="#sex==true">他的</s:if>
					<s:else>她的</s:else>微博</span>
		...
		<tr>
			<td height="772" valign="top"
				style="margin-left: 10px; margin-bottom: 10px; margin-right: 10px; margin-top: 10px; text-align: left"><hr />
							...
							<table width="87%" height="58" border="0" align="center">
								<tr align="center" valign="top">
									<td height="58"
										...
										<div>
											<span
												class="spanForwarderforward<s:property value='#c.Fid'/>">
												<a title="<s:property value="#c.messsage.user.username"/>"
